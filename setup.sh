...
cek=$( curl -sS https://raw.githubusercontent.com/tridebleng/permission/main/ip | awk '{print $2}'  | grep $MYIP )
Name=$(curl -sS https://raw.githubusercontent.com/tridebleng/permission/main/ip | grep $MYIP | awk '{print $4}')
if [[ $cek = $MYIP ]]; then
...
PERMISSION() {
    MYIP=$(wget -qO- ipv4.icanhazip.com);
    IZIN=$(curl -sS https://raw.githubusercontent.com/tridebleng/permission/main/ip | awk '{print $2}' | grep $MYIP)
    if [[ "$MYIP" = "$IZIN" ]]; then
        Bloman
    else
