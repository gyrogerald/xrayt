...
PERMISSION() {
    MYIP=$(wget -qO- ipv4.icanhazip.com);
    IZIN=$(curl -sS https://raw.githubusercontent.com/tridebleng/permission/main/access | awk '{print $3}' | grep $MYIP)
    if [[ "$MYIP" = "$IZIN" ]]; then
        Bloman
    else
...
Exp=$(curl -sS https://raw.githubusercontent.com/tridebleng/permission/main/access | grep $MYIP | awk '{print $3}')
fi
